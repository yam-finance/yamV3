...
async function a() {
  fs.readFile('../delegators.json', 'utf8', function(err, contents) {
    ...
    console.log(web3.utils.toBN(delegators[0]["balanceOfUnderlying"]), twentySeven)
...
async function delegatorRewards(delegators) {
  ...
  for (let i = 0; i < delegators.length; i++) {
    ...
    if (!under && web3.utils.toBN(delegators[i]["balanceOfUnderlying"]).lt(twentySeven)) {
      console.log("adding, UNDER!!!", i)
      a = await migrator.methods.addDelegatorReward(
        dels,
        amts,
        under
      ).send({from: accounts[0], gas: 7000000, gasPrice: gp});
      console.log("a", a);
      under = true;
      dels = []
      amts = []
  ...
  await migrator.methods.addDelegatorReward(
    dels,
    amts,
    under
